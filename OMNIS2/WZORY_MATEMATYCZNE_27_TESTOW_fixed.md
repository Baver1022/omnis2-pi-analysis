...
**Wzór Chi-square:**
```
$\chi^2$ = $\sum_{i=0}^9$ (fᵢ - n/10)$^2$ / (n/10)
```
...
**Z-score:**
```
Z = (runs - E[runs]) / $\sqrt{\text{Var}[runs]}$
```
...
```
$\chi^2$ = $\sum_j$ (ones_per_blockⱼ - block_size/2)$^2$ / (block_size/2)
```
...
**Entropia spektralna:**
```
H_s = -$\sum_k$ (P[k]/ΣP) $\cdot$ $\log_2$(P[k]/ΣP + ε)
```
...
**Dopasowanie modelu:**
```
c = argmin Σ (H_observed(N) - H_model(N, c))$^2$
```
...
```
S_forward[i] = $\sum_j$₌₀ⁱ (2$\cdot$binary[j] - 1)
```

**Backward cumulative sum:**
```
S_backward[i] = $\sum_j$₌ᵢⁿ (2$\cdot$binary[j] - 1)
```
...
**Gdzie:**
```
φ(m) = -Σ_pattern p(pattern) $\cdot$ $\log_2$(p(pattern))
```
...
**Chi-square dla par:**
```
$\chi^2$_pairs = Σ_pair (count_pair - expected)$^2$ / expected
```
...
```
$\chi^2$_triplets = Σ_triplet (count_triplet - expected)$^2$ / expected
```
...
**Chi-square test:**
```
$\chi^2$ = Σ (observed_complexity - expected)$^2$ / expected
```
...
**Z-score:**
```
Z = (f_n - E[f_n]) / $\sqrt{\text{Var}[f_n]/K}$
```
...
**Chi-square:**
```
$\chi^2$ = Σ (observed_spacings - expected)$^2$ / expected
```
...
**Chi-square:**
```
$\chi^2$ = Σ (observed_gaps - expected)$^2$ / expected
```
...
**Chi-square:**
```
$\chi^2$ = $\sum_k$₌₁⁵ (observed(k) - expected(k))$^2$ / expected(k)
```
...
**Z-test:**
```
Z = (observed_mean - E[length]) / (std / $\sqrt{n}$_trials)
```
...
**Z-test:**
```
Z = (observed_mean - E[sum]) / (std / $\sqrt{n}$_blocks)
```
...
**Chi-square:**
```
$\chi^2$ = Σ (observed_ranks - expected)$^2$ / expected
```
...
**Chi-square:**
```
$\chi^2$ = Σ (observed_weights - expected)$^2$ / expected
```
