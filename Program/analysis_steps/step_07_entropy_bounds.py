...
class Step07EntropyBounds(AnalysisStep):
    ...
    def execute(self, digits: np.ndarray, checkpoint_data: Optional[Dict] = None) -> Dict:
        ...
        print("   🔄 Przygotowywanie wartości N do analizy...")
        N_values = []
        current = 1000
        ...
        print(f"   ✅ Wartości N: {len(N_values)} punktów")
        
        # Oblicz entropię dla każdego N
        print("   🔄 Obliczanie entropii dla każdego N...")
        entropy_results = []
        ...
        print(f"   ✅ Obliczono entropię dla {len(entropy_results)} wartości N")
        
        # Fit modelu (jeśli mamy wystarczająco punktów)
        model_fit = None
        if len(entropy_results) >= 3:
            print("   🧮 Dopasowywanie modelu: H(N) = log₂(10) * (1 - c/log(N))...")
            try:
                ...
                print(f"   ✅ Model dopasowany: c = {popt[0]:.6f}")
